...
public:
  ...
  //  void sendPub(TestMessage msg);
  //  int removeSubItem(CStr topic,int linkid);
  //  void addSubItem(CStr topic,int linkid);
   void procPub(TUniNetMsg *msg,set<int> subers);
...
private:
   ...
   //topic-linkid mapping

   //what is the constraint of map key-element
...
public:
  ...
  //  }
   ...
   void setServer(CMqttServerTcp *p);
private:
  ...
  //  int state;
   ...
   CMqttServerTcp *server;
   MqttServerStateMachine *stateMachine;
   CStr mRecvBuffer;
};
