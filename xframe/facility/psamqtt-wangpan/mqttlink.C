...
{
  printf("CMqttLink::msgParse being called");
   ...
   {
      UniERROR("CMqttLink::msgParse, parse length failed,bodyLength: %d,remainLength: %d");
      return -1;
   }

  //  UniINFO("Before calling msgProcess");
...
void CMqttLink::sendPublish(CStr& topic, CStr& content)
{
  ...
  //   printf("%x,",topic.getByte(i));
  //  printf("\n");
  //  printf("content: ");
  //  for(int i=0 ; i<content.length() ; ++i)
  //   printf("%x,",content.getByte(i));
  //  printf("\n");

   CStr body;
   CMqttMsg::appendStr(body, topic.c_str());

  //  printf("encoding topic: ");
  //  for( int i=0 ; i<body.length() ; ++i )
  //   printf("%x,",body.getByte(i));
  //  printf("\n");
...
void CMqttLink::protocolLogic(std::auto_ptr<TUniNetMsg> msg)
{
  UniERROR("calling CMqttLink::protocolLogic method");
}
...
{
   //UniINFO("CMqttLink::invokeService being called");
  ...
  //  UniINFO("CMqttLink::invokeService: before calling getAppKey");
   msg->tAddr.logAddr = mParent->getAppKey(tag);
  //  UniINFO("CMqttLink::invokeService: after calling getAppKey");
  ...
  //  UniINFO("CMqttLink::invokeService before calling protocolLogic");
   ...
   UniINFO("CMqttLink::invokeService: Before calling protocolLogic:");
   protocolLogic(std::auto_ptr<TUniNetMsg>(msg));
   UniINFO("CMqttLink::invokeService: after calling protocolLogic:");
   //task->protocolLogic(std::auto_ptr<TUniNetMsg>(msg));
  //  CUniFrame::sendMsgToFEAM(msg);
}

void CMqttLink::responseService(int msgId, TMsgBody* msgBody)
{
   TUniNetMsg* msg = new TUniNetMsg;
   msg->oAddr.logAddr = g_thisPsaId;
   msg->oAddr.taskInstID = getLinkId();
   ...
   UniINFO("CMqttLink::responseService before protocolLogic");
   protocolLogic(std::auto_ptr<TUniNetMsg>(msg));
   UniINFO("CMqttLink::responseService after protocolLogic");
  //  task->protocolLogic(std::auto_ptr<TUniNetMsg>(msg));
  //  CUniFrame::sendMsgToFEAM(msg);
}
...
{
  //  UniINFO("CMqttServerLink::msgProcess: packetType: %d",packetType);
...
void CMqttServerLink::processSubscribe(const char* p, int length)
{
  ...
  //  UniINFO("CMqttServerLink::processSubscribe:Before paring topic");
   while(length > 0)
   {
      CStr topic;
...
void CMqttClientLink::processSubAck(const char* p, int length)
{
  ...
  //  if(getAppAddr(packetId, addr))
   responseService(MQTT_P_SUBACK,  subAckArg);
}
...
void CMqttClientLink::processUnsubAck(const char* p, int length)
{
  ...
  //  TMsgAddress addr;
  // if(getAppAddr(packetId, addr))
   responseService(MQTT_P_UNSUBACK, unsubAckArg);
}
...
void CMqttClientLink::processConnAck(const char* p, int length)
{
   ...
   if(code == 0)
      mAlive = true;
   TMqttConnAck *conAckArg = new TMqttConnAck;
   conAckArg->code = code;
   responseService(MQTT_P_CONNACK,conAckArg);
   // need to do some extra work to make compatible with other parts
}
