...
bool HttpMessage::encode(DataBuffer *output) {
    ...
    if (_pcode){	//请求包的首行
		output->writeBytes(_req_line, strlen(_req_line));
	    output->writeBytes("\r\n", 2);
	} 
    else 
    {
...
bool HttpMessage::decode(DataBuffer *input, PacketHeader *header) {
    ...
    int len = header->_dataLen;//数据包body长度
	_pcode = header->_pcode;
 ...
 * 设置请求行内容
 */
void HttpMessage::setReqLine(const char *reqline){
	if (_req_line) {
        ::free(_req_line);
        _req_line = NULL;
    }
    if (reqline) {
        _req_line = strdup(reqline);
    }
}
