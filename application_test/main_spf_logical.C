...
char *appName = "Testspflogical";

int main(int argc, char* argv[])
{
    threadEnvCreate();
    InitLogInfo(File, Info, 500, "log/Testspflogical_");
    
    TFRAMEManager* manager = NULL;
    LogTask* 	   log    = NULL;
    TPsaRscTask*   psa     = NULL;
    TTestLogicalTask* test = NULL;

    manager=    new TFRAMEManager();
    psa=        new TPsaRscTask();
    test = new TTestLogicalTask();
    log=       new LogTask();    
   // cout<<"check is null"<<endl;
    if(manager==NULL || psa==NULL || log==NULL || test==NULL)
    {
	UniERROR("Init Testspflogical Error!");
    ...
    manager->registerTask(4,  psa);
    manager->registerTask(5,  test);
